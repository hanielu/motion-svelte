...
const runSpringTests = (unit?: string | undefined) => {
	...
	const parseTestValue = (val: string | number): number => (typeof val === "string" ? parseFloat(val) : val);
	...
	describe(`useSpring ${unit ? `with ${unit}` : "with numbers"}`, () => {
		...
		test("creates a spring that animates to the subscribed motion value", async () => {
			...
			const testNear = (value: string | number, expected: number, deviation = 2) => {
				const numValue = parseTestValue(value);
				expect(numValue >= expected - deviation && numValue <= expected + deviation).toBe(true);
			};
