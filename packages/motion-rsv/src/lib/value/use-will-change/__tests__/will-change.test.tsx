import { frame, motion, MotionGlobalConfig, useMotionValue, useWillChange } from "../../..";
...
describe("willChange", () => {
	test("Renders values defined in animate on initial render", async () => {
		const Component = () => {
			...
			return <motion.div animate={{ x: 100, backgroundColor: "#000" }} style={{ opacity, willChange }} />;
		};
	...
	test("Externally-provided motion values are not added to will-change", async () => {
		...
		expect(container.firstChild).not.toHaveStyle("will-change: height, opacity;");
		expect(container.firstChild).not.toHaveStyle("will-change: opacity, height;");
	});
	...
	test("Add values when they start animating", async () => {
		const Component = ({ animate }: any) => {
			const willChange = useWillChange();
			return <motion.div initial={false} animate={animate} transition={{ duration: 0.1 }} style={{ willChange }} />;
		};
	...
	test("Doesn't remove values when animation interrupted", async () => {
		const Component = ({ animate }: any) => {
			const willChange = useWillChange();
			return <motion.div initial={{ x: 0 }} animate={animate} transition={{ duration: 0.1 }} style={{ willChange }} />;
		};
		const { container, rerender } = render(<Component animate={{}} />);
		await nextFrame();
...
describe("willChange injection", () => {
	...
	test("Renders values defined in animate on initial render", async () => {
		const Component = () => {
			const opacity = useMotionValue(0);
			return <motion.div animate={{ x: 100, backgroundColor: "#000" }} style={{ opacity }} />;
		};
	...
	test("Add values when they start animating", async () => {
		const Component = ({ animate }: any) => {
			return <motion.div initial={false} animate={animate} transition={{ duration: 0.1 }} />;
		};
		const { container, rerender } = render(<Component animate={{}} />);
		await nextFrame();
	...
	test("Doesn't remove values when animation interrupted", async () => {
		const Component = ({ animate }: any) => {
			return <motion.div initial={{ x: 0 }} animate={animate} transition={{ duration: 0.1 }} />;
		};
		const { container, rerender } = render(<Component animate={{}} />);
		await nextFrame();
