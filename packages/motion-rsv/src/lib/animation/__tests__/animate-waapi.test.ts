...
describe("animate() with WAAPI", () => {
	test("Can override transition options per-value", async () => {
		...
		expect(a.animate).toBeCalledWith({ opacity: [0, 1], offset: undefined }, { ...defaultOptions, duration: 1000 });
	...
	test("Can accept timeline sequences", async () => {
		...
		animate([
			"label",
			[[a, b], { opacity: [0, 1], transform: ["scale(0)", "scale(1)"] }, { duration: 1, transform: { duration: 2 } }],
		]);
