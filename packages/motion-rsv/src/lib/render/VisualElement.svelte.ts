...
import { MotionConfigContext, type ReducedMotionConfig } from "../context/motion-config-context.js";
...
import { Previous, read, type ReadableBox } from "runed";
...
> {
  ...
  // This is a raw state because we only ever update it by assignment
  // we also need it to be a state because we need to be able to derive its previous value
  presenceContext = $state.raw<PresenceContextProps | null>(null);
  #prevPresenceContext = new Previous(() => this.presenceContext);

  get prevPresenceContext() {
    return this.#prevPresenceContext.current;
  }
  ...
  // TODO: (haniel) come back to this, it's possible that we need to convert this file to use runes
  constructor(
    {
      parent,
      props,
  ...
  private bindToMotionValue(key: string, value: MotionValue) {
    ...
    const removeOnChange = value.on("change", (latestValue: AnyResolvedKeyframe) => {
  ...
  render = () => {
    ...
    // TODO: (haniel) fix this, making style a string could cause issues
  ...
  getValue(key: string, defaultValue?: AnyResolvedKeyframe | null): MotionValue | undefined {
