...
describe("as function", () => {
	test("sets initial value", async () => {
		...
		expect(container.firstChild).toHaveStyle("transform: translateX(100px) translateY(-100px)");
	});
});
...
describe("as function with multiple values", () => {
	test("sets initial value", async () => {
		const Component = () => {
			...
			const z = useTransform([x, y], ([latestX, latestY]: [number, string]) => latestX * parseFloat(latestY));
			return <motion.div style={{ x, y, z }} />;
		};

		const { container } = render(<Component />);
		expect(container.firstChild).toHaveStyle("transform: translateX(4px) translateY(5px) translateZ(20px)");
	});
});
...
describe("as function with no passed MotionValues", () => {
	test("sets initial value", async () => {
		...
		await new Promise<void>((resolve) => {
			frame.postRender(() => {
				expect(container.firstChild).toHaveStyle("transform: translateX(5px) translateY(5px) translateZ(25px)");
				resolve();
			});
		});
	});
...
test("can be re-pointed to another `MotionValue`", async () => {
	...
	await nextMicrotask();
	expect(container.firstChild as Element).toHaveStyle("transform: translateX(4px)");

	rerender(<Component target={a} />);
	await nextMicrotask();
	expect(container.firstChild as Element).toHaveStyle("transform: translateX(2px)");
});
