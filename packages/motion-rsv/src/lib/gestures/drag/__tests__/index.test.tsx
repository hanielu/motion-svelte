...
import { BoundingBox, motion, motionValue, MotionValue, useWillChange } from "../../../";
...
describe("dragging", () => {
	...
	test.skip("outputs to external values if provided", async () => {
		...
		const Component = () => (
			<MockDrag>
				<motion.div drag _dragX={externalX} _dragY={externalY} style={{ x, y }} />
			</MockDrag>
		);
	...
	test("whileDrag applies animation state", async () => {
		...
		const Component = () => (
			<MockDrag>
				<motion.div drag whileDrag={{ opacity: 0.5 }} transition={{ type: false }} style={{ opacity }} />
			</MockDrag>
		);
	...
	test("impose left drag constraint", async () => {
		...
		const Component = () => (
			<MockDrag>
				<motion.div drag dragConstraints={{ left: -100 }} dragElastic={false} style={{ x, y }} />
			</MockDrag>
		);
	...
	test("impose right drag constraint", async () => {
		...
		const Component = () => (
			<MockDrag>
				<motion.div drag dragConstraints={{ right: 300 }} dragElastic={false} style={{ x, y }} />
			</MockDrag>
		);
	...
	test("impose top drag constraint", async () => {
		...
		const Component = () => (
			<MockDrag>
				<motion.div drag dragConstraints={{ top: -100 }} dragElastic={false} style={{ x, y }} />
			</MockDrag>
		);
	...
	test("impose bottom drag constraint", async () => {
		...
		const Component = () => (
			<MockDrag>
				<motion.div drag dragConstraints={{ bottom: 100 }} dragElastic={false} style={{ x, y }} />
			</MockDrag>
		);
	...
	test("drag constraints can be updated", async () => {
		const x = motionValue(0);
		const y = motionValue(0);
		const Component = ({ constraints }: { constraints: Partial<BoundingBox> }) => (
			<MockDrag>
				<motion.div drag dragConstraints={constraints} dragElastic={false} style={{ x, y }} />
			</MockDrag>
		);

		const { container, rerender } = render(<Component constraints={{ top: -100, bottom: 0 }} />);
	...
	test.skip("updates position when updating drag constraints", async () => {
		...
		const { rerender } = render(<Component constraints={{ right: 42, bottom: 53 }} />);
		...
		rerender(<Component constraints={{ left: 10, top: 14, right: 100, bottom: 100 }} />);
	...
	test.skip("accepts new motion values", async () => {
		...
		expect(container.firstChild).toHaveStyle("transform: translateX(105px) translateY(0px)");
	});
});
