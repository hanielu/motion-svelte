...
function useInitialMotionValues(
  ...
  buildHTMLStyles(state, visualState.current, transformTemplate);
...
function useStyle(
  ...
  const styleProp = props.current.style || {};
  const style = {};
  ...
  if (styleProp && typeof styleProp === "object") {
    copyRawValuesOnly(style, styleProp as any, props.current);
  }

  Object.assign(style, useInitialMotionValues(props.current, visualState));
...
export function useHTMLProps(
  ...
  const htmlProps = $derived.by(() => {
    ...
    if (props.current.drag && props.current.dragListener !== false) {
      ...
      style.touchAction =
        props.current.drag === true ? "none" : `pan-${props.current.drag === "x" ? "y" : "x"}`;
    }
    ...
    const baseStyleString =
      typeof props.current.style === "string" ? (props.current.style as string) : "";
