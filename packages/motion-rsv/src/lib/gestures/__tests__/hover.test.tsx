...
import { pointerDown, pointerEnter, pointerLeave, render } from "../../jest.setup";
...
describe("hover", () => {
	test("hover event listeners fire", async () => {
		...
		const Component = () => <motion.div onHoverStart={hoverIn} onHoverEnd={hoverOut} />;
	...
	test("whileHover applied", async () => {
		const promise = new Promise(async (resolve) => {
			...
			const Component = () => (
				<motion.div whileHover={{ opacity: 0 }} transition={{ type: false }} style={{ opacity }} />
			);
	...
	test("whileHover applied as variant", async () => {
		...
		const promise = new Promise(async (resolve) => {
			...
			const opacity = motionValue(1);
			const Component = () => (
				<motion.div whileHover="hidden" variants={variant} transition={{ type: false }} style={{ opacity }} />
			);
	...
	test("whileHover propagates to children", async () => {
		...
		const promise = new Promise(async (resolve) => {
			...
			const opacity = motionValue(1);
			const Component = () => (
				<motion.div whileHover="hidden" variants={parent} transition={{ type: false }} data-id="hoverparent">
					<motion.div variants={child} style={{ opacity }} transition={{ type: false }} data-id="hoverchild" />
				</motion.div>
			);
	...
	test("whileHover is unapplied when hover ends", () => {
		const promise = new Promise(async (resolve) => {
			...
			const { container } = render(<Component onAnimationComplete={onComplete} />);
