...
describe("useMotionTemplate", () => {
	test("sets initial value", async () => {
		...
		expect(container.firstChild).toHaveStyle(`transform: translateX(1px) translateY(2px)`);
	});
	...
	test("responds to manual setting from parent value", async () => {
		...
		return new Promise<void>((resolve) => {
			requestAnimationFrame(() => {
				expect(firstChild).toHaveStyle(`transform: translateX(10px) translateY(2px)`);
				resolve();
			});
		});
	});
	...
	test("respects static values", async () => {
		...
		expect(container.firstChild).toHaveStyle(`transform: translateX(1px) translateY(1px)`);
