...
function runTests(render: (components: any) => string) {
	test("will-change not applied", () => {
		...
		expect(div).toBe(`<div style="--color:#000;clip-path:inset(10px);transform:translateX(100px)"></div>`);
	});
	...
	test("will-change not set in static mode", () => {
		...
		expect(div).toBe(`<div style="clip-path:inset(10px);transform:translateX(100px)"></div>`);
	});
	...
	test("will-change manually set", () => {
		...
		expect(div).toBe(`<div style="will-change:opacity;--color:#000;transform:translateX(100px)"></div>`);
	});
	...
	test("will-change manually set by MotionValue", () => {
		...
		const div = render(
			<motion.div initial={{ x: 100, "--color": "#000" } as any} animate={{ x: 200 }} style={{ willChange }} />
		);

		expect(div).toBe(`<div style="--color:#000;will-change:opacity;transform:translateX(100px)"></div>`);
	});

	test("will-change correctly not applied when isStatic", () => {
		const div = render(
			<MotionConfig isStatic>
				<motion.div initial={{ x: 100, "--color": "#000" } as any} animate={{ x: 200 }} />
			</MotionConfig>
		);

		expect(div).toBe(`<div style="--color:#000;transform:translateX(100px)"></div>`);
	});
}
