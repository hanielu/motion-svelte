...
import type { HTMLVisualElement } from "../../render/html/HTMLVisualElement.js";
...
import { applyBoxDelta, applyTreeDeltas, transformBox, translateAxis } from "../geometry/delta-apply.js";
...
export function createProjectionNode<I>({
	...
	return class ProjectionNode implements IProjectionNode<I> {
		...
		mount(instance: I) {
			...
			if (this.options.animate !== false && visualElement && (layoutId || layout)) {
				this.addEventListener(
					"didUpdate",
					({ delta, hasLayoutChanged, hasRelativeLayoutChanged, layout: newLayout }: LayoutUpdateData) => {
		...
		setAnimationOrigin(delta: Delta, hasOnlyRelativeTargetChanged: boolean = false) {
		...
		applyProjectionStyles(
			...
			for (const key in scaleCorrectors) {
				...
				if (applyTo) {
					...
					if (isCSSVariable) {
						(this.options.visualElement as HTMLVisualElement).renderState.vars[key] = corrected;
					} else {
