...
describe("createAnimationsFromSequence", () => {
	...
	test("Can set label as first item in sequence", () => {
		const animations = createAnimationsFromSequence(
			["my label", [a, { opacity: 0 }, { duration: 1 }], [b, { y: 500 }, { duration: 1, at: "my label" }]],
			undefined,
			undefined,
			{ spring }
		);
	...
	test("Adds spring as duration-based easing when only one keyframe defined", () => {
		const animations = createAnimationsFromSequence([[a, { x: [0, 100] }, { type: "spring" }]], undefined, undefined, {
			spring,
		});
	...
	test("It correctly repeats easing", () => {
		const animations = createAnimationsFromSequence(
			[[a, { x: [0, 50, 100] }, { duration: 1, repeat: 1, ease: ["easeIn", "easeOut"] }]],
			undefined,
			undefined,
			{ spring }
		);
		...
		expect(ease).toEqual(["easeIn", "easeOut", "linear", "easeIn", "easeOut", "easeIn"]);
	});
	...
	test.skip("It correctly adds repeatDelay between repeated keyframes", () => {
		const animations = createAnimationsFromSequence(
			[[a, { x: [0, 100] }, { duration: 1, repeat: 1, repeatDelay: 0.5 }]],
			undefined,
			undefined,
			{ spring }
		);
	...
	test.skip("It correctly mirrors repeated keyframes", () => {
		const animations = createAnimationsFromSequence(
			[[a, { x: [0, 100] }, { duration: 1, repeat: 3, repeatType: "mirror" }]],
			undefined,
			undefined,
			{ spring }
		);

		expect(animations.get(a)!.keyframes.x).toEqual([0, 100, 100, 0, 0, 100, 100, 0]);
		const { duration, times } = animations.get(a)!.transition.x;
	...
	test.skip("It correctly reverses repeated keyframes", () => {
		const animations = createAnimationsFromSequence(
			[[a, { x: [0, 100] }, { duration: 1, repeat: 3, repeatType: "reverse" }]],
			undefined,
			undefined,
			{ spring }
		);
