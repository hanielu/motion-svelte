<script lang="ts" module>
	...
	>(
		...
		const MotionDOMComponent: Component<MotionComponentProps<Props>> = (anchor, props) => {
			...
			const collector = PresenceCollectorContext.getOr();
	...
	function getProjectionFunctionality(props: MotionProps) {
		...
		return {
			...
			MeasureLayout:
				props.layoutRoot || drag?.isEnabled(props) || layout?.isEnabled(props) ? combined.MeasureLayout : undefined,
...
<UseRender {Component} {props} ref={setRef} latestValues={visualState.latestValues} {isStatic} {forwardMotionProps} />
