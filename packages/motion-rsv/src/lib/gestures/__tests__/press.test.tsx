...
import { motion } from "../..";
import { pointerDown, pointerEnter, pointerLeave, pointerUp, render } from "../../jest.setup";
...
describe("press", () => {
	...
	test("press event listeners fire via keyboard", async () => {
		...
		const Component = () => <motion.div onTapStart={pressStart} onTap={press} onTapCancel={pressCancel} />;
	...
	test("press event listeners are cleaned up", async () => {
		const press = jest.fn();
		const { container, rerender } = render(<motion.div onTap={() => press()} />);
	...
	test("press gesture works with animation state", async () => {
		const [a, b] = await new Promise<[Element, Element]>((resolve) => {
			...
			const Component = () => {
				...
				return (
					<motion.div data-testid="parent" animate={isPressed ? ["pressed"] : ["rest"]}>
						<motion.div data-testid="a" {...childProps} onTapStart={() => setPressedState(true)} />
			...
			setTimeout(() => resolve([getByTestId("a") as Element, getByTestId("b") as Element]), 200);
		});
	...
	test("press gesture variant applies and unapplies with whileHover", () => {
		...
		return expect(promise).resolves.toEqual([0.5, 0.75, 1, 0.75, 0.5, 0.75, 1, 1, 0.5]);
	});
	...
	test("press gesture variant applies and unapplies as state changes", () => {
		const promise = new Promise(async (resolve) => {
			...
			const { container, rerender } = render(<Component isActive={false} />);
		...
		return expect(promise).resolves.toEqual([0.5, 0.75, 1, 1, 1, 1, 1, 1, 1]);
	});
