...
describe("animate", () => {
	...
	test("Applies target keyframe when animation has finished", async () => {
		...
		const animation = animate(div, { opacity: 0.6 }, { duration, x: {}, "--css-var": {} });
		return animation.then(() => {
	...
	test("Works with multiple elements", async () => {
		...
		const animation = animate([div, div2], { opacity: 0.6 }, { duration, x: {}, "--css-var": {} });
	...
	test("Applies final target keyframe when animation has finished, repeat: reverse even", async () => {
		const div = document.createElement("div");
		const animation = animate(div, { opacity: [0.2, 0.5] }, { duration, repeat: 2, repeatType: "reverse" });
		await animation.then(() => {
			expect(div).toHaveStyle("opacity: 0.5");
		});
	});
	...
	test("Applies final target keyframe when animation has finished, repeat: mirror", async () => {
		const div = document.createElement("div");
		const animation = animate(div, { opacity: [0.2, 0.5] }, { duration, repeat: 1, repeatType: "mirror" });
		await animation.then(() => {
			expect(div).toHaveStyle("opacity: 0.2");
		});
	});
	...
	test("Applies final target keyframe when animation has finished, repeat: mirror even", async () => {
		const div = document.createElement("div");
		const animation = animate(div, { opacity: [0.2, 0.5] }, { duration, repeat: 2, repeatType: "mirror" });
		await animation.then(() => {
			expect(div).toHaveStyle("opacity: 0.5");
		});
	});
	...
	test("Is typed correctly", async () => {
		const div = document.createElement("div");
		animate(div, { "--css-var": 0 }, { duration: 1, "--css-var": { duration: 1 } });
		animate(div, { pathLength: 0 }, { duration: 1, pathLength: { duration: 1 } });
