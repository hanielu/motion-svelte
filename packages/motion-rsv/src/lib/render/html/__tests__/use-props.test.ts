...
describe("HTML useProps", () => {
	...
	test("should generate new styles when a new visualState object is passed", () => {
		...
		const { result, rerender } = renderHook(({ state }: any) => useHTMLProps({}, state), {
	...
	test("should generate the correct props when drag is enabled", () => {
		const { result } = renderHook(() => useHTMLProps({ drag: true }, { x: 3 }));
		...
		const { result: resultX } = renderHook(() => useHTMLProps({ drag: "x" }, { x: 3 }));
		...
		const { result: resultY } = renderHook(() => useHTMLProps({ drag: "y" }, { x: 3 }));
