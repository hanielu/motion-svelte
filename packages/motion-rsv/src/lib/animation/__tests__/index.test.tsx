...
describe("useAnimation", () => {
	...
	test("fire's a component's onAnimationComplete", async () => {
		const promise = new Promise((resolve) => {
			const Component = () => {
				...
				return <motion.div animate={animation} style={{ x }} onAnimationComplete={() => resolve(x.get())} />;
			};
	...
	test("fires a component's onAnimationComplete with the animation definition", async () => {
		const promise = new Promise((resolve) => {
			const Component = () => {
				...
				return (
					<motion.div animate={animation} style={{ x }} onAnimationComplete={(definition) => resolve(definition)} />
				);
			};
	...
	test("variants fire a child's onAnimationComplete", async () => {
		const promise = new Promise((resolve) => {
			...
			const Component = () => {
				return (
					<motion.div initial="hidden" animate="visible" variants={variants} transition={{ type: false }}>
						<motion.div
							onAnimationComplete={(definition) => resolve(definition)}
	...
	test("animates to named variants", async () => {
		const promise = new Promise((resolve) => {
			const Component = () => {
				...
				return <motion.div variants={variants} animate={animation} style={{ x }} />;
			};
	...
	test("propagates variants to children", async () => {
		const promise = new Promise((resolve) => {
			const Component = () => {
				...
				useEffect(() => {
					controls.start("foo").then(() => resolve([x.get(), backgroundColor.get()]));
				}, []);

				return (
					<motion.div animate={controls} variants={variants} style={{ x }}>
						<motion.div variants={childVariants} style={{ backgroundColor }} />
					</motion.div>
				);
			};
	...
	it("respects initial even if passed controls", () => {
		const Component = () => {
			const controls = useAnimation();
			return <motion.div animate={controls} initial={{ x: 10, background: "#fff" }} />;
		};
	...
	test("propagates variants to children even if not variants set on controlling component", async () => {
		const promise = new Promise((resolve) => {
			const Component = () => {
				...
				useEffect(() => {
					controls.start("foo").then(() => resolve(backgroundColor.get()));
				}, []);
			...
			const Component = () => {
				...
				return (
					<>
						...
						<motion.div animate={controls} custom={2} style={{ x: xb }} />
					</>
				);
			};
	...
	test(".set updates variants throughout a tree", async () => {
		const promise = new Promise((resolve) => {
			const Component = () => {
				...
				return (
					<motion.div animate={controls} variants={parent} style={{ x: xa }}>
